...
def train_model():
    ...
    fold_results = []
    # 新增：消融实验与 Checkpoint 路径配置
    save_dir = "checkpoints"
    os.makedirs(save_dir, exist_ok=True)

    # Todo:做不同消融实验时，修改这个名字即可
    ...
    for fold, (train_idx, test_idx) in enumerate(kf.split(range(len(dataset)))):
        ...
        # 修改：尝试读取已有的 Latest Checkpoint 实现断点续训
            ...
            for graph, label in train_loader:
                ...
                if molecule_in_batch_counter == BATCH_SIZE_SIMULATED:
                    ...
                    if minibatch_count % 500 == 0:
                        ...
                        print(f"   [Checkpoint] 已自动同步最新进度至: {checkpoint_path}")
        ...
        # 新增：保存当前折的 Final Checkpoint 用于后续组员对齐与评估
        final_checkpoint_path = os.path.join(save_dir, f"{experiment_name}_fold{fold + 1}_final.pt")
        torch.save({
            "fold": fold,
            "minibatch_count": minibatch_count,
            "model_state_dict": model.state_dict(),
