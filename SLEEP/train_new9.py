...
class FocalLoss(nn.Module):
    def __init__(self, alpha=None, gamma=2.0, smoothing=0.05, reduction='mean'):
...
MODEL_SAVE_PATH = "best_sleep_model_v6.pth"
...
weights[1] = weights[1] * 1.85  # 顶格修复 N1
weights[2] = weights[2] * 1.55  # 持续拉升 N2
weights[3] = weights[3] * 0.80  # 削弱 N3
weights[4] = weights[4] * 1.25  # REM
weights = torch.tensor(weights, dtype=torch.float32).to(device)

model = DeepConvNet(chans=CHANNELS, time_point=TIME_POINTS, nb_classes=CLASSES, dropoutRate=0.5).to(device)
criterion = FocalLoss(alpha=weights, gamma=2.0, smoothing=0.05)
